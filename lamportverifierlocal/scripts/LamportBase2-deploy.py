from brownie import accounts, LamportBase2
...
def main():
    ...
    contract2 = LamportBase2.deploy({'from': brownie_account, 'gas_limit': 3000000})
    print(f"LamportBase2 deployed: {contract2.address}")
    #contract = AnonIDContract.deploy({'from': accounts[0]})
    #print(f"AnonID deployed: {contract.address}")


    with open('contract_LamportBase2.txt', 'w') as file:
